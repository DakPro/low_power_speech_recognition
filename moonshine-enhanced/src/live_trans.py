import time
from queue import Queue

import numpy as np
from silero_vad import VADIterator, load_silero_vad
from sounddevice import InputStream

from moonshine_onnx import MoonshineOnnxModel, load_tokenizer

MODEL = "moonshine/base"

SAMPLING_RATE = 16000

CHUNK_SIZE = 512  # Silero VAD requirement with sampling rate 16000.
LOOKBACK_CHUNKS = 5
LOOK_BACK_SIZE = LOOKBACK_CHUNKS * CHUNK_SIZE
MAX_LINE_LENGTH = 80

# These affect live caption updating - adjust for your platform speed and model.
MAX_SPEECH_SECS = 15
MIN_REFRESH_SECS = 0.2
STATS = False


class Transcriber(object):
    def __init__(self, model_name: str, rate: int = 16000):
        if rate != 16000:
            raise ValueError("Moonshine supports sampling rate 16000 Hz.")
        self.model = MoonshineOnnxModel(model_name=model_name)
        self.rate = rate
        self.tokenizer = load_tokenizer()

        self.inference_secs = 0
        self.number_inferences = 0
        self.speech_secs = 0
        self.__call__(np.zeros(int(rate), dtype=np.float32))  # Warmup.

    def __call__(self, speech: np.ndarray) -> str:
        """Returns string containing Moonshine transcription of speech."""
        self.number_inferences += 1
        self.speech_secs += len(speech) / self.rate
        start_time = time.time()

        tokens = self.model.generate(speech[np.newaxis, :].astype(np.float32))
        text = self.tokenizer.decode_batch(tokens)[0]

        self.inference_secs += time.time() - start_time
        return text


class TranscriptionProcess(object):
    def __init__(self):

        self.transcribe = Transcriber(model_name=MODEL, rate=SAMPLING_RATE)
        self.vad_iterator = VADIterator(
            model=load_silero_vad(onnx=True),
            sampling_rate=SAMPLING_RATE,
            threshold=0.5,
            min_silence_duration_ms=300,
        )
        self.q = Queue()
        self.caption_cache = []

        def input_callback(data, frames, time, status) -> None:
            if status:
                print(status)
            self.q.put((data.copy().flatten(), status))

        self.input_callback = input_callback

    def end_recording(self, speech: np.ndarray, do_print=True) -> None:
        """Transcribes, prints and caches the caption then clears speech buffer."""
        text = self.transcribe(speech)
        if do_print:
            self.print_captions(text)
        self.caption_cache.append(text)
        speech *= 0.0

    def soft_reset(self) -> None:
        """Soft resets Silero VADIterator without affecting VAD model state."""
        self.vad_iterator.triggered = False
        self.vad_iterator.temp_end = 0
        self.vad_iterator.current_sample = 0

    def print_captions(self, text) -> None:
        """Prints right justified on same line, prepending cached captions."""
        if len(text) < MAX_LINE_LENGTH:
            for caption in self.caption_cache[::-1]:
                text = caption + " " + text
                if len(text) > MAX_LINE_LENGTH:
                    break
        if len(text) > MAX_LINE_LENGTH:
            text = text[-MAX_LINE_LENGTH:]
        else:
            text = " " * (MAX_LINE_LENGTH - len(text)) + text
        print("\r" + (" " * MAX_LINE_LENGTH) + "\r" + text, end="", flush=True)

    def start(self, print_transcriptions: bool = False) -> None:
        stream = InputStream(
            samplerate=SAMPLING_RATE,
            channels=1,
            blocksize=CHUNK_SIZE,
            dtype=np.float32,
            callback=self.input_callback,
        )
        stream.start()

        speech = np.empty(0, dtype=np.float32)
        recording = False

        with stream:
            try:
                while True:
                    chunk, status = self.q.get()
                    # if status:
                    #     process_speech(status)
                    speech = np.concatenate((speech, chunk))

                    if not recording:
                        speech = speech[-LOOK_BACK_SIZE:]

                    speech_dict = self.vad_iterator(chunk)
                    if speech_dict:
                        if "start" in speech_dict and not recording:
                            recording = True
                            start_time = time.time()

                        if "end" in speech_dict and recording:
                            recording = False
                            self.end_recording(speech)

                    elif recording:
                        # Possible speech truncation can cause hallucination.

                        if (len(speech) / SAMPLING_RATE) > MAX_SPEECH_SECS:
                            recording = False
                            self.end_recording(speech)
                            self.soft_reset()

                        if (time.time() - start_time) > MIN_REFRESH_SECS:
                            if print_transcriptions:
                                self.print_captions(self.transcribe(speech))
                            start_time = time.time()

            except Exception as e:
                print(e)
                stream.close()
